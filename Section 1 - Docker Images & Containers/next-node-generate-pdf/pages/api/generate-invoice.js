...
export default async (req, res) => {
	...
	const { name, company, email, invoiceId, tripId, tripTotal, vat, totalDue, createdAt, dueOn } = JSON.parse(req.body);
	...
	try {
		...
		const html = template({
			...
			email,
			company,
			createdAt: dayjs.unix(createdAt).format("MMMM D, YYYY"),
			dueOn: dayjs.unix(dueOn).format("MMMM D, YYYY"),
			invoiceId,
			tripId,
			tripTotal,
			vat,
